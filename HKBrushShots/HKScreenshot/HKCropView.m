...
- (instancetype)initWithImageView:(UIImageView *)imageView {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        // 设置裁剪区域
        _clipRect = CGRectMake((frame.size.width-MIN(frame.size.width/2, frame.size.height/2))/2,
                               (frame.size.height-MIN(frame.size.width/2, frame.size.height/2))/2,
                               MIN(frame.size.width/2, frame.size.height/2),
                               MIN(frame.size.width/2, frame.size.height/2));
...
#pragma mark 获取剪切图片-如果截图位置需微调此处作修改
- (UIImage *)getClipImageRect:(CGRect)newRect {
    ...
    CGRect clipCGRect = CGRectMake((newRect.origin.x) * scale,
                                   (newRect.origin.y) * scale,
                                   (newRect.size.width) * scale,
                                   (newRect.size.height) * scale);
    NSLog(@"x=%f,y=%f,w=%f,h=%f",newRect.origin.x,newRect.origin.y,newRect.size.width,newRect.size.height);
